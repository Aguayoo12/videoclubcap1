...
class CatalogController extends Controller
{
    ...
    public function getIndex(){
        return view("catalog.index")->with("peliculas", Movie::get());
    }

    public function getShow($id = 0){
        return view("catalog.show")->with("pelicula", Movie::findOrFail($id))->with("id", $id);
    }
    ...
    public function putRent($id){
        ...
        $movie->rented = 1;
        $movie->save();
        return redirect('catalog/show/' . $id)->with('msg',1)->with("pelicula", Movie::findOrFail($id));
    }
